...
public class Segment {
    ...
    public DoublePair getMinimumAndMaximumY() {
        double minY = Math.min(start.getY(), finish.getY()),               maxY = Math.max(start.getY(), finish.getY());
        return new DoublePair(minY, maxY);
    }
    public DoublePair getMinimumAndMaximumX() {
        double minX = Math.min(start.getX(), finish.getX()),               maxX = Math.max(start.getX(), finish.getX());
        return new DoublePair(minX, maxX);
    }
    public double getZOnX(double currentX) {
        DoublePair xes = getMinimumAndMaximumX();
        if (currentX <= xes.getMinValue() || xes.getMaxValue() < currentX) {        return -Double.MAX_VALUE;       }
